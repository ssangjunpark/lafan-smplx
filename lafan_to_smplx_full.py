...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        smplx_kwargs = {
            "betas": betas[args.model_type].to(device).view(1, -1).expand(n_frames, -1),
    ...
    if args.output_file is not None:
        ...
        np.savez(
            args.output_file,
            gender="neutral",
            surface_model_type=args.model_type,
        ...
        print(f"Saved SMPL-X data to {args.output_file}")
        print(f"  - Number of frames: {n_frames}")
        print(f"  - Frame rate: {LAFAN1_FRAME_RATE} fps")
        print(f"  - Duration: {mocap_time_length:.2f} seconds")
        print(f"  - Model type: {args.model_type}")
        print(f"  - Pose shape: {poses_flat.shape}")
        print(f"  - Trans shape: {trans_tensor.shape}")
