...
def process_single_bvh(bvh_file, output_file, smpl_model_path, gender, rerun):
    """Process a single BVH file and convert it to SMPL format."""
    try:
    ...
    print(f"Saved SMPL data to {output_file}")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Folder to save the output .npz files",
    )
    parser.add_argument(
        "--gender",
        type=str,
    ...
    if args.rerun:
        import rerun as rr
        rr.init("lafan_to_smpl_all", spawn=True)
